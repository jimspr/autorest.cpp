...
namespace AutoRest.Cpp.Model
{
    public class MethodCpp : Method
    {
        ...
        // {
        //     return this.GetAsyncMethodParameterDeclaration(false, true);
        // }
        ...
        public virtual string GetSyncMethodParameterDeclaration(bool addCustomHeaderParameters, bool addDefault)
        {
            ...
            foreach (var parameter in LocalParameters)
            {
                ...
                {
                    string format = "{0} {1}";
                    declarations.Add(string.Format(CultureInfo.InvariantCulture, format, parameter.ModelTypeName, parameter.Name));
                }
            }
            ...
            {
                if (addDefault)
                    declarations.Add("const std::map<std::string, std::string>& customHeaders = std::map<std::string, std::string>{}");
                else
                    declarations.Add("const std::map<std::string, std::string>& customHeaders");
            }
        ...
        /// <param name="addCustomHeaderParameters">If true add the customHeader to the parameters</param>
        /// <returns>Generated string of parameters</returns>
        public virtual string GetAsyncMethodParameterDeclaration(bool addCustomHeaderParameters, bool addDefault)
        {
            var declarations = this.GetSyncMethodParameterDeclaration(addCustomHeaderParameters, addDefault);
            ...
            if (addDefault)
                declarations += "const cancellation_token_t& cancellationToken = cancellation_token_t{}";
            else
                declarations += "const cancellation_token_t& cancellationToken";
        ...
        /// </summary>
        [JsonIgnore]
        public IEnumerable<ParameterCpp> LocalParameters
        {
            get
            {
                return
                    Parameters.Where(parameter =>
                        ...
                        .OrderBy(item => !item.IsRequired).Cast<ParameterCpp>();
            }
        }

        /// <summary>
        /// Get the set of possible return types
        /// </summary>
        public HashSet<string> GetSetOfReturnTypes()
        {
            ...
            if (Responses != null)
            {
                ...
                {
                    if (r.Value.Body != null)
                    {
                        set.Add(r.Value.Body.DeclarationName);
                    }
                }
            }
            if ((DefaultResponse != null) && (DefaultResponse.Body != null))
            {
                set.Add(DefaultResponse.Body.DeclarationName);
            }
            return set;
        }
        ...
        public void AddReferencedCompositeTypes(HashSet<string> set)
        {
            if (Responses != null)
            {
                foreach (var r in Responses)
                {
                    AddIfCompositeOrEnum(set, r.Value.Body);
                }
            }
            if ((DefaultResponse != null) && (DefaultResponse.Body != null))
            {
                AddIfCompositeOrEnum(set, DefaultResponse.Body);
            }
            AddIfCompositeOrEnum(set, ReturnType.Body);
            AddIfCompositeOrEnum(set, ReturnType.Headers);

            foreach (var parameter in LocalParameters)
            {
                var type = parameter.ModelType;
                AddIfCompositeOrEnum(set, type);
            }
        ...
        public virtual string OperationResponseReturnTypeString
        {
            get
            {
                var set = GetSetOfReturnTypes();
                if (set.Count() > 1)
                {
                    return $"http_response<std::variant<" + string.Join(",", set.ToArray()) + ">>";
                }
                if (ReturnType.Body != null && ReturnType.Headers != null)
                {
                    return $"http_response<std::variant<{ReturnType.Body.DeclarationName},{ReturnType.Headers.DeclarationName}>>";
                }
                if (ReturnType.Body != null)
                {
                    return $"http_response<{ReturnType.Body.DeclarationName}>";
                }
                if (ReturnType.Headers != null)
                {
                    return $"http_response<{ReturnType.Headers.DeclarationName}>";
                }
        ...
        public virtual string ReturnTypeString
        {
            get
            {
                ...
                if (set.Count() > 1)
                {
                    return "std::variant<" + string.Join(",", set.ToArray()) + ">>";
                }
                if (ReturnType.Body != null)
                {
                    return ReturnType.Body.DeclarationName;
                }
                if (ReturnType.Headers != null)
                {
                    return ReturnType.Headers.DeclarationName;
                }
                else
                {
                    return "void";
        ...
        /// </summary>
        [JsonIgnore]
        public ParameterCpp RequestBody => Body as ParameterCpp;
...
//            auto pos = _url.find("{petId}");
//            _url.replace(pos, pos + strlen({petId}), to_string(petId));

            foreach (var pathParameter in this.LogicalParameters.Where(p => p.Location == ParameterLocation.Path))
            {
                string replaceString = "replace_in_string({0}, \"{{{1}}}\", escape_data_string(curl, {2}));";
                if (pathParameter.SkipUrlEncoding())
                {
                    replaceString = "replace_in_string({0}, \"{{{1}}}\", {2});";
                }
            ...
            {
                builder.AppendLine("std::vector<std::string> _queryParameters;");
                foreach (var queryParameter in this.LogicalParameters.Where(p => p.Location == ParameterLocation.Query))
                {
                    if ((queryParameter as ParameterCpp).IsNullable())
                    {
                        builder.AppendLine("if ({0}.has_value())", queryParameter.Name)
                            .AppendLine("{").Indent();
                    }

                    var replaceString = (queryParameter.SkipUrlEncoding()) ?
                        "_queryParameters.push_back(string_format(\"{0}={{0}}\", {1}));" :
                        "_queryParameters.push_back(string_format(\"{0}={{0}}\", escape_data_string(curl, {1})));";
                ...
                builder.AppendLine("if (!_queryParameters.empty())")
                    .AppendLine("{").Indent();
                if (this.Extensions.ContainsKey("nextLinkMethod") && (bool)this.Extensions["nextLinkMethod"])
                {
                    builder.AppendLine("{0} += ({0}.Contains(\"?\") ? \"&\" : \"?\") + string_join(\"&\", _queryParameters);", variableName);
                }
                else
                {
                    builder.AppendLine("{0} += \"?\" + string_join(\"&\", _queryParameters);", variableName);
                }
