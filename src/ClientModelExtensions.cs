...
namespace AutoRest.Cpp
{
    public static class ClientModelExtensions
    {
        ...
        public static string GetFormattedReferenceValue(this Parameter parameter, string clientReference)
        {
            ...
            if (parameter.IsNullable()) // std::optional needs .value() added
                paramvalue += ".value()";

            SequenceType sequence = parameter.ModelType as SequenceType;
            if (sequence == null)
            {
                return parameter.ModelType.ToString(clientReference, paramvalue);
            }
            ...
            return string.Format(CultureInfo.InvariantCulture,
                "string_join(\"{0}\", {1})", parameter.CollectionFormat.GetSeparator(), paramvalue);
        }
        ...
        public static string ToString(this IModelType type, string clientReference, string reference)
        {
            ...
            return string.Format(CultureInfo.InvariantCulture, "autorest::util::to_string({0})", reference);
        }
        ...
        public static string CheckNull(string valueReference, string executionBlock)
        {
            ...
            sb.AppendLine("if ({0}.has_value())", valueReference)
                .AppendLine("{").Indent()
            ...
            Dictionary<Constraint, string> constraints)
        {
            ...
            var model = type as CompositeTypeCpp;
            var sequence = type as SequenceTypeCpp;
            var dictionary = type as DictionaryTypeCpp;

            var sb = new IndentedStringBuilder();

            if (model != null && model.ShouldValidateChain())
            {
                sb.AppendLine("// TODO - handle nested validation");
//                sb.AppendLine("{0}.value().Validate();", valueReference);
//                sb.AppendLine("{0}.second.Validate();", valueReference);
            }
            ...
            if (sequence != null && sequence.ShouldValidateChain())
            {
                ...
                {
                    sb.AppendLine("for (auto& {0} : {1})", elementVar, valueReference)
                        .AppendLine("{").Indent()
            ...
            else if (dictionary != null && dictionary.ShouldValidateChain())
            {
                ...
                if (!string.IsNullOrEmpty(innerValidation))
                {
                    sb.AppendLine("for (auto& {0} : {1})", valueVar, valueReference)
                        .AppendLine("{").Indent()
                            .AppendLine(innerValidation).Outdent()
                        .AppendLine("}").Outdent();
                }
        ...
        private static string ToLiteral(string input)
        {
                ...
                {
                    using (var provider = CodeDomProvider.CreateProvider("Cpp"))
                    {
            ...
            foreach (var constraint in constraints.Keys)
            {
                ...
                switch (constraint)
                {
                    ...
                    case Constraint.MaxItems:
                        constraintCheck = $"{valueReference}.size() > {constraintValue}";
                        break;
...
//                            constraintCheck = $"!System.Linq.Enumerable.All({valueReference}.Values, value => System.Text.RegularExpressions.Regex.IsMatch(value, {constraintValue}))";
                            constraintCheck = $"0/*TODO*/";
                        }
                        else
                        {
                            constraintCheck = $"!regex_is_match({valueReference}, {constraintValue})";
                        }
                        break;
                ...
                if (constraintCheck != null)
                {
                    if (constraint != Constraint.UniqueItems)
                    {
                        sb.AppendLine("if ({0})", constraintCheck)
                            .AppendLine("{").Indent()
                            .AppendLine("throw validation_exception_t{};")
                            .Outdent()
                            // .AppendLine("throw validation_exception_t(Microsoft.Rest.ValidationRules.{0}, \"{1}\", {2});",
                    ...
                    else
                    {
                        sb.AppendLine("if ({0})", constraintCheck)
                            ...
                            .AppendLine("throw validation_exception_t{};")
                            .Outdent()
                            // .AppendLine("throw validation_exception_t(Microsoft.Rest.ValidationRules.{0}, \"{1}\");",
        ...
        // public static string AsNullableType(this IModelType modelType) => modelType.IsValueType() ? $"optional<{modelType.Name}>" : modelType.DeclarationName;
        public static string AsNullableType(this IModelType modelType) => $"optionalZ<{modelType.DeclarationName}>";
        ...
        // public static string AsNullableType(this IModelType modelType, Func<bool> predicate) => predicate() && modelType.IsValueType() ? $"optional<{modelType.Name}>" : modelType.DeclarationName;
        public static string AsNullableType(this IModelType modelType, Func<bool> predicate) => predicate() ? $"optionalX<{modelType.DeclarationName}>" : modelType.DeclarationName;
        /// <summary>
        ...
        // public static string AsNullableType(this IModelType modelType, bool predicate) => predicate && modelType.IsValueType() ? $"optional<{modelType.Name}>" : modelType.DeclarationName;
        // public static string AsNullableType(this IModelType modelType, bool predicate) => predicate && modelType.IsValueType() ? $"optionalY<{modelType.Name}>" : modelType.DeclarationName;
        public static string AsNullableType(this IModelType modelType, bool predicate) => predicate ? $"std::optional<{modelType.DeclarationName}>" : modelType.DeclarationName;
        ...
        public static bool IsNullable(this IVariable variable) => (variable.IsXNullable ?? !variable.IsRequired);
    }
}